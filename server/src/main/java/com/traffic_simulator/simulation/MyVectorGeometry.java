...
public class MyVectorGeometry {
    ...
    public static Coordinates computeRoadVectorCoordinates(Road road) {
        return new Coordinates(
                road.getEndCoordinate().getX() - road.getStartCoordinate().getX(),
                road.getEndCoordinate().getY() - road.getStartCoordinate().getY());
    }
    ...
    public static boolean pointIsOnRoad(Coordinates point, Road road) {
        if (MyVectorGeometry.computeVectorsAngleCos(
                MyVectorGeometry.computeVectorCoordinates(road.getStartCoordinate(), point),
                MyVectorGeometry.computeRoadVectorCoordinates(road)) == 1) {
            return road.getStartCoordinate().getX() < point.getX() & point.getX() < road.getEndCoordinate().getX() ||
                    road.getEndCoordinate().getX() < point.getX() & point.getX() < road.getStartCoordinate().getX();
        }
